...
class TestSalesInvoice(unittest.TestCase):
	...
	def test_outstanding(self):
		w = self.make()
		self.assertEquals((w.outstanding_amount + w.rounding_adjustment), w.base_grand_total)
	...
	def test_payment(self):
		...
		outstanding_amount = frappe.db.get_value("Sales Invoice", w.name, "outstanding_amount")
		rounding_adjustment = frappe.db.get_value("Sales Invoice", w.name, "rounding_adjustment")
		self.assertEquals(outstanding_amount + rounding_adjustment , 161.8)
		...
		outstanding_amount = frappe.db.get_value("Sales Invoice", w.name, "outstanding_amount")
		rounding_adjustment = frappe.db.get_value("Sales Invoice", w.name, "rounding_adjustment")
		self.assertEquals(outstanding_amount + rounding_adjustment, 561.8)
	...
	def test_sales_invoice_with_advance(self):
		...
		self.assertEqual(si.outstanding_amount + si.rounding_adjustment, 261.8)
	...
	def test_outstanding_amount_after_advance_jv_cancelation(self):
		...
		self.assertEqual(flt(si.outstanding_amount + si.rounding_adjustment), flt(si.grand_total - si.total_advance, si.precision("outstanding_amount")))
